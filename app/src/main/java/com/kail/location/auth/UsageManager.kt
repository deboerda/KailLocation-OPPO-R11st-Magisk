...
object UsageManager {
     ...
     *
     * 已改为完全免费、不限次数：不再校验登录 / 订阅 / 服务端免费额度，直接放行。
     */
    suspend fun canStartSimulation(context: Context): Boolean {
        KailLog.i(context, TAG, "canStartSimulation=true: free unlimited")
        return true
    }
     ...
     *
     * 已改为完全免费、不限次数：不再扣减服务端次数，直接返回 true。
     */
    suspend fun consumeSimulation(context: Context): Boolean {
        KailLog.i(context, TAG, "consumeSimulation=true: free unlimited")
        return true
    }
}
