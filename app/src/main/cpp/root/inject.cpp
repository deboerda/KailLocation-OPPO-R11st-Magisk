...
static uint32_t gRemoteLoaderDlopen = 0;
static uint32_t gRemoteLoaderDlsym  = 0;
static uint32_t gRemoteCallerAddr   = 0;
static uint32_t gRemoteReturnAddressOverride = 0;
...
static uint32_t findLibraryNonExecutableAddress(const char *libraryPath, int pid);
...
static uint32_t findLibraryNonExecutableAddress(const char *libraryPath, int pid) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    char perms[5] = {0};
    if (sscanf(line, "%x-%x %4s", &start, &end, perms) != 3)
      continue;
...
static uint32_t callRemoteFunction(uint32_t func, int argc, ...) {
  ...
  regs.uregs[14] = gRemoteReturnAddressOverride; // zero or NX caller address
...
static int injectLibraryIntoProcess(int pid, const char *libraryPath, const char *entryArg) {
  ...
  if (localLoaderDlopen) {
    gRemoteLoaderDlopen = resolveRemoteSymbolAddress(
        linkerPath, (uint32_t)(uintptr_t)localLoaderDlopen);
  }
  if (localLoaderDlsym) {
    gRemoteLoaderDlsym = resolveRemoteSymbolAddress(
        linkerPath, (uint32_t)(uintptr_t)localLoaderDlsym);
  }
  ...
  printf("inject diag: loader_dlopen=0x%x loader_dlsym=0x%x caller=0x%x mode=%s\n",
         gRemoteLoaderDlopen, gRemoteLoaderDlsym, gRemoteCallerAddr,
