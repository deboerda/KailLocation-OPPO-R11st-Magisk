...
// Android O's libdl wrappers infer the caller namespace from LR. Our ptrace
// trampoline uses LR=0, so normal dlopen/dlsym calls land in "(anonymous)".
// These loader entry points accept an explicit caller address instead.
static uint64_t gRemoteLoaderDlopen = 0;
static uint64_t gRemoteLoaderDlsym  = 0;
static uint64_t gRemoteCallerAddr   = 0;
static uint64_t gRemoteReturnAddressOverride = 0;
...
static uint64_t findLibraryNonExecutableAddress(const char *libraryPath, int pid);
...
// Return an address in a readable but non-executable PT_LOAD mapping.  It is
// useful as a synthetic LR: libdl recognizes the address as belonging to libc
// (and therefore the default namespace), then returning to it reliably raises
// SIGSEGV because the page is NX.  The ptrace trampoline already uses that
// SIGSEGV as its normal remote-call completion signal.
static uint64_t findLibraryNonExecutableAddress(const char *libraryPath, int pid) {
  char mapsPath[256] = {0};
  int target = (pid == -1) ? gTargetPid : pid;
  snprintf(mapsPath, sizeof(mapsPath), "/proc/%d/maps", target);
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (sscanf(line, "%llx-%llx %4s", &start, &end, perms) != 3)
      continue;
    bool pathMatches = strstr(line, libraryPath) != nullptr ||
                       (baseName && strstr(line, baseName) != nullptr);
    if (pathMatches && perms[0] == 'r' && perms[2] != 'x' && end > start) {
...
static uint64_t callRemoteFunction(uint64_t func, int argc, ...) {
  ...
  regs[30] = gRemoteReturnAddressOverride; // zero or an NX caller-address page
  regs[32] = func;     // pc
...
static int injectLibraryIntoProcess(int pid, const char *libraryPath, const char *entryArg) {
  ...
  // libc belongs to system_server's default namespace. The loader only uses
  // this address to identify the caller's containing soinfo; it is not called.
  gRemoteCallerAddr = findLibraryNonExecutableAddress(libcPath, gTargetPid);
  if (!gRemoteCallerAddr)
    gRemoteCallerAddr = gRemoteCalloc;
  ...
  // Android O exposes synthetic libdl symbols backed by the dynamic linker.
  // Apply the linker ASLR slide to translate their local addresses into the
  // corresponding addresses in system_server.
  void *localLoaderDlopen = dlsym(RTLD_DEFAULT, "__loader_dlopen");
  void *localLoaderDlsym  = dlsym(RTLD_DEFAULT, "__loader_dlsym");
  if (localLoaderDlopen) {
    gRemoteLoaderDlopen = resolveRemoteSymbolAddress(
        linkerPath, (uint64_t)localLoaderDlopen);
  }
  if (localLoaderDlsym) {
    gRemoteLoaderDlsym = resolveRemoteSymbolAddress(
        linkerPath, (uint64_t)localLoaderDlsym);
  }
  ...
  printf("inject diag: loader_dlopen=0x%llx loader_dlsym=0x%llx caller=0x%llx mode=%s\n",
         (unsigned long long)gRemoteLoaderDlopen,
         (unsigned long long)gRemoteLoaderDlsym,
         (unsigned long long)gRemoteCallerAddr,
         (gRemoteLoaderDlopen && gRemoteCallerAddr) ? "explicit-loader" :
         (gRemoteCallerAddr ? "spoofed-lr" : "legacy-wrapper"));
  ...
  if (gRemoteLoaderDlopen && gRemoteCallerAddr) {
    remoteHandle = callRemoteFunction(gRemoteLoaderDlopen, 3,
                                      remotePath, (uint64_t)RTLD_NOW,
                                      gRemoteCallerAddr);
  } else {
    gRemoteReturnAddressOverride = gRemoteCallerAddr;
    remoteHandle = callRemoteFunction(gRemoteDlopen, 2,
                                      remotePath, (uint64_t)RTLD_NOW);
    gRemoteReturnAddressOverride = 0;
  }
  ...
  if (remoteHandle) {
    ...
    if (gRemoteLoaderDlsym && gRemoteCallerAddr) {
      uint64_t remoteSymbolName = writeRemoteString("doRun");
      if (remoteSymbolName) {
        doRunRemoteByDlsym = callRemoteFunction(gRemoteLoaderDlsym, 3,
                                                remoteHandle, remoteSymbolName,
                                                gRemoteCallerAddr);
        callRemoteFunction(gRemoteFree, 1, remoteSymbolName);
      }
    } else if (gRemoteDlsym) {
      uint64_t remoteSymbolName = writeRemoteString("doRun");
      if (remoteSymbolName) {
        gRemoteReturnAddressOverride = gRemoteCallerAddr;
