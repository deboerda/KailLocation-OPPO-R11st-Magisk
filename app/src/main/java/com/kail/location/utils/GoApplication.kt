...
class GoApplication : Application(), Application.ActivityLifecycleCallbacks {
    ...
    override fun attachBaseContext(base: Context?) {
        ...
        // BlackBox 沙箱（NativeCore.disableHiddenApi/disableResourceLoading +
        // System.loadLibrary("blackbox")）在 Android 8.1（API 27）上会在进程启动早期
        // 破坏资源加载，导致 Compose 渲染拿不到资源 → 白屏无崩溃。沙箱仅供
        // nav_sandbox（应用双开）使用，与所有位置/路线/WiFi/基站/相机模拟无关；
        // 故在低版本上跳过 BlackBox 初始化，回到无沙箱启动路径（与 1.5.1 一致）。
        // 8.1 以上仍保留沙箱功能。
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.P) {
            android.util.Log.w("GoApplication", "Skipping BlackBox sandbox init on low API (SDK=${android.os.Build.VERSION.SDK_INT}) to avoid white-screen on 8.1")
            return
        }
