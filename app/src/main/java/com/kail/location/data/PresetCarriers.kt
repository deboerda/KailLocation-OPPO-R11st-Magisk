package com.kail.location.data
...
object PresetCarriers {
    ...
    data class CarrierPreset(
        val name: String,        // 本地名称
        val displayName: String, // 英文显示名
        val region: String       // 地区代码
    )
     ...
     * 按地区分组获取运营商
     */
    fun getByRegion(region: String): List<CarrierPreset> {
        return presets.filter { it.region == region }
    }
    ...
    fun getRegions(): List<String> {
        return presets.map { it.region }.filter { it.isNotEmpty() }.distinct()
    }

    /**
     * 搜索运营商
     */
    fun search(query: String): List<CarrierPreset> {
        val lowerQuery = query.lowercase()
        return presets.filter { 
            it.name.lowercase().contains(lowerQuery) ||
            it.displayName.lowercase().contains(lowerQuery) ||
            it.region.lowercase().contains(lowerQuery)
        }
    }
}
